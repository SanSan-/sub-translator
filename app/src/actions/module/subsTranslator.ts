...
export const translate = (
  ...
  threadCount = 1, maxLen = 7000, maxCount = 50
  ...
  switch (opts.api) {
    case TranslatorApiType.YANDEX: {
      ...
      while (i < lines.length) {
        ...
        if (temp.join(EMPTY_STRING).length >= maxLen || i - start >= maxCount) {
    ...
    case TranslatorApiType.GOOGLE: {
      for (let i = 0; i < lines.length; i += maxCount) {
        await dispatch(translateByGoogleMulti(lines.slice(i, i + maxCount), opts));
      }
      break;
    }
    default: {
